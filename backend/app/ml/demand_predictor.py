"""
Demand Predictor — Phase 3.2: Predictive Supply & Demand Engine
===============================================================
Pipeline summary:
  1. LOAD  — Read historical_demand.csv (synthetic, Jan–Dec 2025)
  ...
  6. PREDICT — Predict future requested_quantity for org / category / date
...
DEMAND_CSV = DATA_DIR / "historical_demand.csv"
MODEL_DIR = Path(__file__).resolve().parent / "artifacts"
DEMAND_MODEL_PATH = MODEL_DIR / "demand_rf_model.pkl"
DEMAND_META_PATH = MODEL_DIR / "demand_rf_meta.pkl"
...
# Feature engineering
# ---------------------------------------------------------------------------
def _build_demand_features(df: pd.DataFrame,
                           org_enc: LabelEncoder,
                           cat_enc: LabelEncoder,
                           is_training: bool = True) -> pd.DataFrame:
    """
    Build the full feature matrix from the raw demand DataFrame.
    ...
    Features (no leakage — all derived from information available before prediction):
      ...
      - rolling_7d_mean  : per-(org, category) 7-day rolling mean of past requested quantities
      - rolling_7d_std   : per-(org, category) 7-day rolling std  of past requested quantities
      - lag_7            : requested_quantity 7 days prior for same (org, category)
    ...
    grp = feat.groupby(["organization_id", "category"])["requested_quantity"]
...
class DemandPredictor:
    """
    Explainable ML predictor for community resource demand quantities.

    Uses a RandomForestRegressor trained on synthetic historical demand data
    ...
    def __init__(self):
        ...
        if not self._load():
            logger.info("No persisted demand model found — training now.")
            self.train()
    ...
    def _save(self) -> None:
        ...
        with open(DEMAND_META_PATH, "wb") as f:
            pickle.dump(meta, f)
        logger.info("Demand model saved to %s", DEMAND_MODEL_PATH)

    def _load(self) -> bool:
        if DEMAND_MODEL_PATH.exists() and DEMAND_META_PATH.exists():
            try:
                with open(DEMAND_MODEL_PATH, "rb") as f:
                    self.model = pickle.load(f)
                with open(DEMAND_META_PATH, "rb") as f:
                    meta = pickle.load(f)
                ...
                logger.info("Demand model loaded from disk (MAE=%.2f, RMSE=%.2f)",
                            self.metrics.get("mae", 0), self.metrics.get("rmse", 0))
                return True
            except Exception as e:
                logger.warning("Failed to load demand model: %s — retraining.", e)
        return False
    ...
    def train(self) -> Dict[str, Any]:
        """
        Load historical demand data, engineer features, apply time-based
        ...
        if not DEMAND_CSV.exists():
            raise FileNotFoundError(f"Historical demand CSV not found: {DEMAND_CSV}")

        df = pd.read_csv(DEMAND_CSV)
        df["date"] = pd.to_datetime(df["date"])
        df = df.dropna(subset=["requested_quantity", "organization_id", "category"])

        # --- Feature engineering (fits encoders on full dataset) ---
        feat = _build_demand_features(df, self.org_enc, self.cat_enc, is_training=True)
        ...
        logger.info(
            "Demand model trained | train=%d rows | val=%d rows | MAE=%.4f | RMSE=%.4f",
    ...
    def predict(
        ...
        """
        Predict demand quantity (kg) for a given receiver org/category/date.
        ...
        if not self.is_trained or self.model is None:
            raise RuntimeError("Demand predictor is not trained yet.")
        ...
        if len(grp) >= 7:
            lag_7 = float(grp["requested_quantity"].iloc[-7])
            roll_mean = float(grp["requested_quantity"].tail(7).mean())
            roll_std  = float(grp["requested_quantity"].tail(7).std(ddof=0))
        elif len(grp) > 0:
            lag_7 = float(grp["requested_quantity"].mean())
            roll_mean = float(grp["requested_quantity"].mean())
            roll_std  = float(grp["requested_quantity"].std(ddof=0) if len(grp) > 1 else 0.0)
        else:
            # Completely unknown org/cat — use global mean as fallback
            global_df = pd.read_csv(DEMAND_CSV)
            lag_7 = float(global_df["requested_quantity"].mean())
            roll_mean = lag_7
            roll_std  = float(global_df["requested_quantity"].std())
        ...
        pred = max(0.0, round(pred, 2))  # Demand cannot be negative
    ...
    def forecast(
        ...
        """Return day-by-day demand predictions for `days` consecutive dates."""
        results = []
